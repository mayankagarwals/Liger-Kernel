...
def run_ddp_test(rank, world_size, mlp_type, config, dtype, num_shards):
    ...
    try:
        ...
        '''
        Those three lines are PyTorch distributed collectives. torch.distributed.gather(tensor, gather_list=None, dst=0) sends the local tensor from every rank to the dst rank; only dst supplies gather_list (a preallocated list of tensors of the same shape/dtype, length world_size) to receive the results. Non-dst ranks pass gather_list=None and just send. It returns None; on rank 0 the gate_grads/up_grads/down_grads lists are populated with the tensors gathered from all ranks. Use this when only one rank needs the data; use all_gather if every rank needs the full list.
        '''
