...
L
L the number of layers in the model. Note that looking at the equation, we can see that the term that will dominate with large hidden dimensions is the 
h
2
h 
2
...
c
c
grad_acc) as much as possible since data parallelism is inherently parallel, unlike the sequential nature of gradient accumulation. Gradient accumulation is then added on top of data parallelism to achieve the target global batch size, when scaling data parallelism alone is not sufficient before you run out of GPUs.
...
If you're using FP32 gradient accumulation with ZeRO-2 or ZeRO-3, you would need to add an additional 
4
Ψ
N
d
...
d
s
num_attention_heads query heads but only 
n
u
m
_
...
In the DeepSeek-V3 technical report
[2]
...
b
s
mbs) to aim for an optimal balance between max global batch size, model size, compute, and communication.
Benchmarking thousands of configurations
...
Additionally, the synchronization patterns when overlapping computation and communication may not always be optimal for our parallel strategies. You can find an example in this blog post by the PyTorch team.
...
+
1
M+1, thread 2 accesses 
M
+
2
M+2, and so forth, the GPU hardware coalesces or combines these requests into one large, efficient access request for the DRAM burst, rather than handling each access individually.
...
=
0
i=0. These elements are not stored close to each other in memory, and this misalignment will be present at each iteration, thereby preventing memory accesses from being coalesced.
...
We also remove a large part of the naive impact of the 
O
(
S
2
...
−5.734×10 
7
 , where we first have the sign, followed by the mantissa and the exponent. As such, we can represent numbers across a wide range of magnitudes with an adaptive precision. Although float32 is the default, there are a range of floating-point formats available in PyTorch:
...
10 
−3
 , and for bfloat it's 10x higher still.
...
[3]
 came up with three tricks to match float32 training:
...
[2]
. The authors carefully analyzed each operation of the forward pass (Fprop) as well as the activation (Dgrad) and weight (Wgrad) backward passes. Similar to BF16 mixed precision training, some aggregations and master weights are kept in higher precision while the operations themselves are performed in FP8.
...
−
1
N−1 times during both the ReduceScatter and AllGather steps. Each GPU sends 
K
N
N
K
...
An AllReduce operation can be broken down into a ReduceScatter followed by an AllGather. The communication cost for these two operations is half that of the AllReduce, which is approximately 
K
K.
...
b
s
seq⋅mbs tokens, where 
m
b
s
mbs is the micro-batch size and 
s
e
q
...
h
seq⋅mbs⋅h elements.
Model weights and gradients: Each weight matrix in your model (e.g. linear layer) contains about 
h
2
h 
2
  elements. Gradients have the same size as weights.
Optimizer states: For each weight matrix (of 
h
2
h 
2
  elements), an optimizer like Adam with mixed precision training will keep momentum and variance states in FP32 precision (
2
×
2
h
...
2h 
2
 ). So, the total number of optimizer states will be around (
6
h
2
6h 
...
m
s
2⋅num_tokens⋅num_params. The backward pass compute is twice that: 
4
⋅
n
u
...
n
2
6⋅seq_len⋅num_params+12⋅num_layers⋅h⋅seq_len 
2
  which accounts for the quadratic scaling from attention operations across the entire sequence, but to simplify the math, we assume that 
s
e
q
_
...
h
h and tensor parallelism degree 
t
p
tp, not on sequence length or batch size.
...
h
2⋅seq⋅mbs⋅h bytes
Total communication per micro-batch: 
4
⋅
s
e
...
h
h, the number of layers in the next pipeline stage, and the ratio of compute to P2P bandwidth capabilities of the hardware.
